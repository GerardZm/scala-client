...
class ApplicationSpec extends PlaySpecification {
  ...
  "Application" should {
	...
	/*"send HTTP POST request containing a FILE and receive response" in new WithServer {
		...
		val file = new File(s"test/utilities/unitTest.txt")
		val postBuilder = asyncHttpClient.preparePost("127.0.0.1:9001")
		val builder = postBuilder.addBodyPart(new FilePart("code", file))
		val request = builder.build()
		val upload = controller.upload(request)
		status(upload) must equalTo(OK)
	}*/
  }
}
